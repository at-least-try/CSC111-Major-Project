...
from rmp_course_dataset import load_course_prof_ratings_csv
...
def create_app() -> Flask:
    ...
    if RATINGS_PATH.exists():
        ratings_index = load_course_prof_ratings_csv(RATINGS_PATH)
    else:
        ratings_index = {}
...
<body>
  ...
  <div class="panel">
    ...
    <p id="status-message" style="min-height:16px;margin:0 0 8px 0;font-size:12px;color:#5f6368;">
      Click a node once to show links (red = prerequisites, green = unlocks). Click the same node again to 
      toggle completion.
    </p>
    {{ plot_html|safe }}
  </div>
  ...
  <script>
    (function () {
      ...
      if (selectedNode) {
        ...
        setStatus(
          "Showing links for " + selectedNode + " (red = prerequisites, green = unlocks). 
